	...
	if(!empty($messages))
	{
		foreach($messages as $msg)
		{
			if(!empty($msg['Description']))
			{
				...
				if($msg['Sent_By']===$User_Id)
				{
					$Full_Name = ucfirst($msg['To_User_First_Name']).' '.ucfirst($msg['To_User_Last_Name'])." (".$desc.")";
					$Url = base_url('chat/show/'.$msg['Sent_To'].'/'.$msg['Thread'].'/'.$msg['Thread_Type'].'/'.$msg['Sent_By']);
				...
				else
				{
					...
					$Url = base_url('chat/show/'.$msg['Sent_By'].'/'.$msg['Thread'].'/'.$msg['Thread_Type'].'/'.$msg['Sent_To']);
			...
			<li class="media media-sm <?php echo ($unread_counter==0)?'unread_msg':'read_msg'; ?>">
				...
				if($msg['Archived']==1)
				{
					...
					<div class="media-footer">
						<label style="cursor:context-menu;" class="btn btn-xs btn-info pull-right">Archived</label>
					</div>                    
					<?php
				}
				?>
